...
k = 1  # Sensor range for alien detection
...
def place_entities(grid, D, k, grid_layout):
    ...
    bot_pos, crew_pos1, alien_pos = None, None, None
    crew_pos2 = None
    ...
    while crew_pos2 is None or crew_pos2 == crew_pos1:
        potential_pos = (np.random.randint(0, D), np.random.randint(0, D))
        if grid[potential_pos] == EMPTY and grid_layout[potential_pos] and manhattan_distance(potential_pos[0], potential_pos[1], bot_pos[0], bot_pos[1]) > k:
...
def detect_beep(crew_pos1, crew_pos2, bot_pos):
    """Simulates beep detection with a probability based on distance using the Manhattan distance."""
    distance1 = manhattan_distance(crew_pos1[0], crew_pos1[1], bot_pos[0], bot_pos[1])
    distance2 = manhattan_distance(crew_pos2[0], crew_pos2[1], bot_pos[0], bot_pos[1])
    beep_prob1 = np.exp(-alpha * (distance1 - 1))
    beep_prob2 = np.exp(-alpha * (distance2 - 1))
    return np.random.uniform(0, 1.0) < max(beep_prob1, beep_prob2)
...
def update_beliefs(bot_pos, alien_pos, crew_pos1,  crew_pos2, D, k, alpha, prob_alien, prob_crew, grid_layout):
    alien_detected = detect_alien(bot_pos, alien_pos)
    beep_detected = detect_beep(crew_pos1, crew_pos2, bot_pos)
    ...
    for x in range(D):
        for y in range(D):
            ...
            if beep_detected:
                # If a beep is detected, the probability is now the joint probability of either crew member being in the cell
                prob_crew[x, y] *= (1 - (1 - np.exp(-alpha * (manhattan_distance(x, y, crew_pos1[0], crew_pos1[1]) - 1))) *
                                          (1 - np.exp(-alpha * (manhattan_distance(x, y, crew_pos2[0], crew_pos2[1]) - 1))))
            else:
                # If no beep is detected, the probability of either crew member being in the cell is reduced
                prob_crew[x, y] *= (np.exp(-alpha * (manhattan_distance(x, y, crew_pos1[0], crew_pos1[1]) - 1)) *
                                          np.exp(-alpha * (manhattan_distance(x, y, crew_pos2[0], crew_pos2[1]) - 1)))
...
def simulate(D, k, alpha):
    ...
    bot_alive = True
    crew_rescued = [False, False]  # Track rescue status of both crew members
    crew_positions = [crew_pos1, crew_pos2]
    ...
    while bot_alive and not all(crew_rescued) and steps < 1000:
        ...
        # Check if the bot is adjacent to either crew member and update the rescue status
        for i, crew_pos in enumerate(crew_positions):
            if manhattan_distance(bot_pos[0], bot_pos[1], crew_pos[0], crew_pos[1]) == 1 and not crew_rescued[i]:
                print(f"Crew member {i+1} rescued by bot at position {bot_pos} in {steps} steps.")
                crew_rescued[i] = True
                prob_crew[crew_pos] = 0  # Update probabilities to reflect rescue


        if crew_rescued[0]:
            next_step, path = choose_next_move(bot_pos, prob_alien, prob_crew[crew_positions[1]], D, grid_layout)
        elif crew_rescued[1]:
            next_step, path = choose_next_move(bot_pos, prob_alien, prob_crew[crew_positions[0]], D, grid_layout)
