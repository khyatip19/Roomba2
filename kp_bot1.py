...
# alphas = np.linspace(0, 1, 11)  # Alpha values from 0 to 1 with 0.1 increments
# k = range(3, 7)  # k values from 3 to 6
...
def simulate(D, k_value, alpha_value, grid, grid_layout, prob_alien, prob_crew, bot_pos, crew_pos, alien_pos):
    ...
    #return steps, crew_rescued, 1 if crew_rescued else 0  # Return number of steps, success status, and crew rescued count
    ...
    for k_value in k_range:  # Iterate over each value in the k_range
        for alpha_value in alpha_range:
            ...
            for _ in range(num_simulations):
                ...
                steps, bot_alive, crew_rescued = simulate(D, k_value, alpha_value, grid, grid_layout, prob_alien, prob_crew, bot_pos, crew_pos, alien_pos)  # And here

                steps_list.append(steps)
                success_count += int(bot_alive and crew_rescued)
                crew_rescued_count += int(crew_rescued)

            avg_steps = np.mean(steps_list)
            success_rate = success_count / num_simulations
            avg_crew_rescued = crew_rescued_count / num_simulations
            ...
            results.append({
                ...
                'avg_crew_rescued': avg_crew_rescued
            })
