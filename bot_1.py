...
def simulate(D, k, alpha):
    ...
    success = bot_alive and crew_rescued
    return success, steps, crew_rescued
...
if __name__ == "__main__":
    num_trials = 20  # Number of trials for simulation
    alpha_values = np.linspace(2, 2, 8)  # Range of alpha values to test
    
    performance_data = {'Alpha': [], 'AverageSteps': [], 'SuccessRate': [], 'CrewRescued': []}
    
    for alpha in alpha_values:
        success_count = 0
        total_steps = 0
        crew_rescued_count = 0
        for _ in range(num_trials):
            # Reset and run the simulation
            # Note: Your simulate function must return the necessary outcome data
            success, steps, crew_rescued = simulate(D=15, k=3, alpha=alpha)
            
            # Collect data based on the simulation results
            if success:
                success_count += 1
            if crew_rescued:
                crew_rescued_count += 1
            total_steps += steps
        
        # Calculate averages and success rate
        average_steps = total_steps / num_trials
        success_rate = success_count / num_trials
        crew_rescued_rate = crew_rescued_count / num_trials
        
        # Store the collected data
        performance_data['Alpha'].append(alpha)
        performance_data['AverageSteps'].append(average_steps)
        performance_data['SuccessRate'].append(success_rate)
        performance_data['CrewRescued'].append(crew_rescued_rate)
    ...
    sns.lineplot(data=performance_df, x='Alpha', y='SuccessRate')
    plt.title('Bot 1 Success Rate by Alpha')
    plt.xlabel('Alpha (Crew Detection Sensitivity)')
    plt.ylabel('Success Rate')
    plt.show()

    # Plotting Average Steps
    plt.figure(figsize=(10, 6))
    sns.lineplot(data=performance_df, x='Alpha', y='AverageSteps')
    plt.title('Bot 1 Average Steps by Alpha')
    plt.xlabel('Alpha (Crew Detection Sensitivity)')
    plt.ylabel('Average Steps Taken')
    plt.show()

    # Plotting Crew Rescued Rate
    plt.figure(figsize=(10, 6))
    sns.lineplot(data=performance_df, x='Alpha', y='CrewRescued')
    plt.title('Bot 1 Crew Rescued Rate by Alpha')
    plt.xlabel('Alpha (Crew Detection Sensitivity)')
    plt.ylabel('Crew Rescued Rate')
    plt.show()
