# bot7.py - Two Alien, Two Crew Member
# Bot 7: Bot 7 is Bot 1, with correct modeling and updates for the
# fact that there are now two aliens and two crew members.
...
def update_beliefs(bot_pos, alien_pos1, alien_pos2, crew_pos1, crew_pos2, D, k, alpha, prob_alien1, prob_alien2, prob_crew, grid_layout, crew_rescued):   
    ...
    for x in range(D):
        for y in range(D):
            ...
            beep_prob1 = np.exp(-alpha * distance) if not crew_rescued[0] else 0
            beep_prob2 = np.exp(-alpha * distance) if not crew_rescued[1] else 0

            # Combine the beep probabilities
            # combined_beep_prob = beep_detected1 + beep_detected2 - (beep_detected1 * beep_detected2)
            combined_beep_prob = beep_prob1 + beep_prob2 - beep_prob2*beep_prob1
            ...
            if beep_detected:
                ...
                prob_crew[x, y] = prob_crew[x, y]*combined_beep_prob
            else:
                # Decrease probability for cells outside beep range
                prob_crew[x, y] = prob_crew[x, y]*(1 - combined_beep_prob)

    # Normalize only non-blocked cells
    total_prob_alien1 = np.sum(prob_alien1[grid_layout == EMPTY])
    total_prob_alien2 = np.sum(prob_alien2[grid_layout == EMPTY])
    total_prob_crew = np.sum(prob_crew[grid_layout == EMPTY])
    if total_prob_alien1 > 0:
        prob_alien1[grid_layout == EMPTY] /= total_prob_alien1
    if total_prob_alien2 > 0:
        prob_alien2[grid_layout == EMPTY] /= total_prob_alien2
    ...
    new_prob_alien1 = np.zeros((D, D))
    for x in range(D):
        for y in range(D):
            if prob_alien1[x, y] > 0:
                adjacent_open_cells = get_adjacent_open_cells(x, y, grid_layout)
                distributed_prob = prob_alien1[x, y] / len(adjacent_open_cells) if adjacent_open_cells else prob_alien1[x, y]
                for adj_x, adj_y in adjacent_open_cells:
                    new_prob_alien1[adj_x, adj_y] += distributed_prob

    # Update the alien belief matrix if there are any probabilities to diffuse
    if np.sum(new_prob_alien1) > 0:
        prob_alien1[:] = new_prob_alien1 / np.sum(new_prob_alien1)

    new_prob_alien2 = np.zeros((D, D))
    for x in range(D):
        for y in range(D):
            if prob_alien2[x, y] > 0:
                adjacent_open_cells = get_adjacent_open_cells(x, y, grid_layout)
                distributed_prob = prob_alien2[x, y] / len(adjacent_open_cells) if adjacent_open_cells else prob_alien2[x, y]
                for adj_x, adj_y in adjacent_open_cells:
                    new_prob_alien2[adj_x, adj_y] += distributed_prob

    # Update the alien belief matrix if there are any probabilities to diffuse
    if np.sum(new_prob_alien2) > 0:
        prob_alien2[:] = new_prob_alien2 / np.sum(new_prob_alien2)
...
def choose_next_move(bot_pos, prob_alien1, prob_alien2, prob_crew, D, grid_layout):
    ...
    path = find_path_with_risk_assessment(bot_pos,goal_pos,ship_layout=grid_layout,risk_scores=(prob_alien1+prob_alien2 - prob_alien1*prob_alien2))
    if path:
        return path[0], path  # Return the next step and the full path
    else:
...
def visualize_grid(grid, grid_layout, prob_alien1, prob_alien2, prob_crew, bot_pos, alien_positions, crew_positions, k, path, crew_rescued):
   ...
   # Append aliens in legend handles
    ...
    ax2.set_title("Alien Probability 1")
    ax2.imshow(prob_alien1, cmap='Reds')
    plt.colorbar(mappable=ax2.imshow(prob_alien1, cmap='Reds'), ax=ax2)

    # Alien Probability 2
    ax2 = plt.subplot(1, 4, 3)
    ax2.set_title("Alien Probability 2")
    ax2.imshow(prob_alien2, cmap='Greens')
    plt.colorbar(mappable=ax2.imshow(prob_alien2, cmap='Greens'), ax=ax2)
...
def simulate(D, k, alpha):
    ...
    prob_alien1 = np.full((D, D), 1/(D*D - 3))  # Initial belief about the first alien's location
    prob_alien2 = np.full((D, D), 1/(D*D - 3))  # Initial belief about the second alien's location
    ...
    while bot_alive and not all(crew_rescued): 
        ...
        update_beliefs(bot_pos, alien_pos1, alien_pos2, crew_pos1, crew_pos2, D, k, alpha, prob_alien1, prob_alien2, prob_crew, grid_layout, crew_rescued)
        
        # If the bot is adjacent to the crew member, rescue immediately
        next_step, path = choose_next_move(bot_pos, prob_alien1, prob_alien2, prob_crew, D, grid_layout)
        ...
        if bot_pos == alien_pos1 or bot_pos == alien_pos2:
            ...
            prob_alien1[bot_pos[0], bot_pos[1]] = 0
            prob_alien2[bot_pos[0], bot_pos[1]] = 0
        ...
        #visualize_grid(grid, grid_layout, prob_alien1, prob_alien2, prob_crew, bot_pos, [alien_pos1, alien_pos2], [crew_pos1, crew_pos2], k, path, crew_rescued)
